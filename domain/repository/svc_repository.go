...
func (u *SvcRepository) InitTable() error {
	return u.mysqlDb.Set("gorm:table_options", "ROW_FORMAT=DYNAMIC").CreateTable(&model.Svc{}, &model.SvcPort{}).Error
}
...
func (u *SvcRepository) DeleteSvcByID(svcID int64) error {
	...
	if tx.Error != nil {
		common.Error("DeleteSvcByID DB transaction start error")
		return tx.Error
	}
